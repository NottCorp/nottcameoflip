...
def invert(
    ...
    *,
    ...
    set_date_resolver=None,
) -> InvertedDataset:
    species = _known_species(entries)
    base_species_count = len(species)
    ...
    log.debug(
        "species pool: %d from cameo entries + %d discovered from card names",
        base_species_count, len(species) - base_species_count,
    )
    resolve_date = (
        set_date_resolver if set_date_resolver is not None else (lambda _: None)
    )
    cards: dict[str, Card] = {}
    for e in entries:
        ...
        if key not in cards:
            cards[key] = Card(
                ...
                release_date=resolve_date(e.set_name),
            )
    ...
    resolved_dates = sum(1 for c in cards.values() if c.release_date)
    log.debug(
        "inverted %d entries → %d cards (%d with resolved release_date)",
        len(entries), len(cards), resolved_dates,
    )
